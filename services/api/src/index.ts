...
async function ensureDefaultOrgData(orgId: string) {
  const now = Date.now();
  // Run all seed inserts + counts in parallel — cross-region latency makes
  // sequential awaits painfully slow (~250ms per round trip).
  const integrations = [
  ...
  const [vendorRows, invoiceRows] = await Promise.all([
    sql<{ count: string }[]>`SELECT COUNT(*)::bigint AS count FROM vendors WHERE org_id = ${orgId}`,
    sql<{ count: string }[]>`SELECT COUNT(*)::bigint AS count FROM invoices WHERE org_id = ${orgId}`,
    sql`
